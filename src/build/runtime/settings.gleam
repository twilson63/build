...
pub fn interpret(effect: settings.Effect) -> Nil {
  case effect {
    settings.PersistSettings(provider, api_key, ollama_url, model) ->
      persist_settings(settings.provider_to_string(provider), api_key, ollama_url, model)
...
fn persist_settings(provider: String, api_key: String, ollama_url: String, model: String) -> Nil
