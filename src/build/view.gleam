...
pub fn view(app: model.Model) -> Element(msg.Msg) {
  ...
  html.div([attribute.class("app")], [
    html.aside([attribute.class("panel chat")], [
      html.header([], [title_row(), html.p([], [html.text("Use an agent to create your application in a live browser workspace.")])]),
      build_project_nav.view(app.project.project_name, app.project.save_status, app.project.name_editing, busy),
      build_settings_modal.view(app.settings),
      build_projects_modal.view(app.project, busy),
      build_element_picker.view(result_to_option(app.preview.selected_element), app.preview.element_comment, busy),
      build_agent_chat.view(app.chat.messages, app.chat.prompt, running, busy),
    ]),
    html.main([attribute.class("workspace")], [
      build_preview.view(app.preview.preview_url, app.preview.selecting_element, running),
      html.section([attribute.class("bottom")], [
        ...
        build_terminal.view(app.webcontainer.logs, app.project.project_ready && !running && app.preview.preview_url != ""),
      ]),
    ]),
  ])
}
  ...
  html.div(
    ...
    list.map(files, fn(file) {
      html.button(
        [
          attribute.type_("button"),
          attribute.class(case file.path == selected_path { True -> "active" False -> "" }),
...
fn selected_file(app: model.Model) {
  case list.find(app.project.files, fn(file) { file.path == app.project.selected_path }) {
    Ok(file) -> file
    Error(_) -> case app.project.files {
      [first, ..] -> first
